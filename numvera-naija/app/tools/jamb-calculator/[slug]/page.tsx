...
import { institutionalCutoff, cutoffSourceLabel, NATIONAL_MIN } from "@/lib/cutoff-resolve";
import { courses, findCourseCutoff } from "@/lib/courses";
...
export default function UniversityJambPage() {
  ...
  const meetsGeneral = uni ? utmeNum >= institutionalCutoff(uni) : false;
    ...
    >
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2">
        <div className="card p-5">
          <p className="text-xs font-bold muted">INSTITUTIONAL UTME MINIMUM</p>
          <p className="mt-2 text-3xl font-extrabold text-brand-700">{institutionalCutoff(uni)}+</p>
          <p className="mt-1 text-sm muted">{cutoffSourceLabel(uni)}</p>
          <p className="mt-1 text-xs muted">National university floor {NATIONAL_MIN.university}+ (2026)</p>
        </div>
        <div className="card p-5">
          ...
          <p className="mt-1 text-sm muted">
            You entered {utmeNum || "—"} · institutional min {institutionalCutoff(uni)}+
          </p>
        </div>
      </div>
      ...
      <div className="mt-10">
        <h2 className="text-xl font-extrabold">Course-specific pages</h2>
        <p className="mt-1 text-sm muted">SEO-friendly pages for popular programmes at {uni.shortName}.</p>
      ...
      <p className="mt-6 text-xs muted">{siteConfig.cutoffsAsOf}</p>
      <div className="mt-10">
